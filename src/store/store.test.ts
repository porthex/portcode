...
describe("resolvePermission", () => {
  ...
  it("restores the owned prompt and settles when local permission IPC rejects", async () => {
    ...
    expect(m.resolvePermission).toHaveBeenCalledWith(pending.id, "allow", false);
  ...
  it("retains the prompt and settles when native delivery fails so the user can retry", async () => {
    ...
    await expect(useStore.getState().resolvePermission("allow")).resolves.toBeUndefined();
  ...
  it("rejects stale identities and clears the exact run-owned Codex prompt", async () => {
    const pending = { id: "p-run", tool: "fs_edit", summary: "x", input: {} };
    useStore.setState({
      sessions: [session({ id: "a" })],
      activeId: "a",
      runs: {
    ...
    await useStore.getState().resolvePermission("a", "stale-id", "deny");
    expect(m.resolvePermission).not.toHaveBeenCalled();
    expect(useStore.getState().runs.a.pendingPermission).toEqual(pending);

    await useStore.getState().resolvePermission("a", pending.id, "deny");
    expect(m.resolvePermission).toHaveBeenCalledWith(pending.id, "deny", false);
    expect(useStore.getState().runs.a.pendingPermission).toBeNull();
    expect(useStore.getState().pendingPermission).toBeNull();
  });
});

describe("resolveCodexRequest", () => {
  it("fails closed on the phone and ignores stale request identities", async () => {
    const request = {
      id: "request-1",
    ...
    useStore.setState({
      activeId: "a",
      remoteMode: true,
      runs: { a: { ...runState(), pendingCodexRequest: request } },
      pendingCodexRequest: request,
    });

    await expect(
      useStore.getState().resolveCodexRequest("a", request.id, { answers: {} }),
    ).rejects.toThrow("only be answered on the desktop");
    expect(m.resolveCodexRequest).not.toHaveBeenCalled();

    useStore.setState({ remoteMode: false, runs: { a: runState() }, pendingCodexRequest: null });
    await useStore.getState().resolveCodexRequest("a", "stale", { answers: {} });
    expect(m.resolveCodexRequest).not.toHaveBeenCalled();
  });
});
