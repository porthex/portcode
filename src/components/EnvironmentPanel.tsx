  ...
  return (
    <div className="border-b border-border pb-1.5">
      <FactRow
        ...
        label="Changes"
        detail={changeDetail}
        actionLabel={`Review changes, ${changeDetail}`}
        onActivate={() => setWorkspaceSurface("review")}
        value={
          repository ? (
...
function FactRow({
  ...
  const className =
    "grid min-h-9 w-full grid-cols-[22px_minmax(0,1fr)_auto] items-center px-0.5 py-1 text-left";
  if (onActivate) {
    return (
      <button
        type="button"
        ...
        onClick={onActivate}
        className={`${className} rounded outline-none transition-colors hover:bg-accent-2/[0.04] focus-visible:bg-accent-2/[0.06] focus-visible:ring-2 focus-visible:ring-accent-2/20`}
      >
        {content}
      </button>
    );
