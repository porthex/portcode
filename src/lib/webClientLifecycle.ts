...
export function startWebClientLifecycle(opts: WebClientLifecycleOptions = {}): () => void {
  ...
  void (async () => {
    ...
    // Durable storage is best-effort; a blocked/corrupt IndexedDB must not create
    // an unhandled rejection or prevent the rest of the lifecycle from starting.
  });
  ...
  // idempotent themselves, while the lifecycle owner may teardown defensively.
  let stopped = false;
  return () => {
    if (stopped) return;
    stopped = true;
...
async function persistPinnedPeer(
  ...
  try {
    ...
    void storage.requestPersistentStorage().catch(() => {});
    ...
    // Storage is best-effort; never leak a rejected promise from this fire-and-forget path.
  }
}
