...
async function loadViteConfig() {
  ...
  // Vite's `defineConfig` can return a UserConfig directly or a (env) => config
  // factory; unwrap both. Cast through `unknown` because the static type is the
  // object form, so the factory branch would otherwise narrow to `never`.
  const raw = mod.default as unknown;
  const cfg =
    typeof raw === "function" ? (raw as (env: Record<string, unknown>) => unknown)({}) : raw;
  return cfg as {
    build?: {
