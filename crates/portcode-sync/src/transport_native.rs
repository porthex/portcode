//! Phone Sync — the NATIVE (desktop) iroh QUIC transport.
//!
//! Moved verbatim from the old `src-tauri/src/sync/transport.rs`: it carries the
//! application-layer Noise session (`super::noise`) over an iroh bidirectional
//! stream, so the relay only ever sees ciphertext. iroh gives us dial-by-public-
//! key P2P with hole-punching + a self-hostable relay fallback and QUIC's
//! connection migration (survives Wi-Fi↔cellular handoff); the Noise layer on top
//! keeps the relay blind.
//!
...
//!
//! `#[cfg(not(target_arch = "wasm32"))]`-only: browsers have no UDP and cannot
//! bind an iroh `Endpoint` directly; the browser sibling is `transport_wasm.rs`.
...
pub async fn connect_and_pair(
    ...
    nonce: &[u8],
) -> Result<Paired<SecureChannel>, String> {
    let conn = endpoint
        .connect(peer, ALPN)
        .await
        .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    //
    // NOTE: the desktop's `SyncHub` (a tauri-coupled wrapper) does not live in
    // this crate, so this moved test drives a plain `tokio::sync::broadcast`
    // channel — which is exactly what `SyncHub` wraps and what `forward_live`
    // consumes — keeping the proof identical without the Tauri dependency.
    #[tokio::test(flavor = "multi_thread")]
    async fn split_channels_forward_live_and_dispatch_commands_over_iroh() {
        use crate::protocol::RemoteCommand;
        use crate::session::{forward_live, handle_commands, CommandHandler};
