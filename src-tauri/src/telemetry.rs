...
mod tests {
    ...
    // ── New Phase-2 consent-file tests ──────────────────────────────────────────

    #[test]
    fn consent_is_live_when_file_contains_one() {
        // Positive case: the exact content "1" arms the gate.
        let path = temp_consent_path();
    ...
    fn consent_is_live_with_surrounding_whitespace() {
        // Whitespace-only wrapping: " 1 " should still be accepted.
        let path = temp_consent_path();
        std::fs::write(&path, " 1 ").unwrap();
        assert!(consent_is_live());
        clear_consent_override(&path);
    ...
    #[test]
    fn consent_is_off_for_empty_string() {
    ...
    #[test]
    fn set_consent_false_when_no_file_does_not_panic() {
        // Removing a file that doesn't exist should be handled gracefully.
        let path = temp_consent_path();
        // path does not exist — set_consent(false) must not panic.
        set_consent(false);
        assert!(!consent_is_live());
        clear_consent_override(&path);
    }
    ...
    #[test]
    fn set_consent_creates_parent_directory() {
        // set_consent(true) must create the parent dir if absent, so the write
        // succeeds even on a fresh installation with no config dir yet.
        let base = std::env::temp_dir().join(format!(
            "portcode-test-mkdir-{}-{:?}",
            std::process::id(),
            std::thread::current().id()
        ));
        let consent_file = base.join("sub").join("consent.flag");
        // Make sure the tree doesn't exist yet.
        let _ = std::fs::remove_dir_all(&base);
        CONSENT_PATH_OVERRIDE.with(|c| *c.borrow_mut() = Some(consent_file.clone()));

        set_consent(true);

        // The parent dir must have been created and the file must contain "1".
        assert!(
            consent_file.exists(),
            "consent file should have been created"
        );
        assert_eq!(
    ...
    #[test]
    fn init_desktop_with_minidump_returns_none_when_no_dsn() {
        // In dev/contributor/test builds SENTRY_DSN is never set, so the function
        // must return None immediately — no SDK init, no monitor, no side effects.
        // (This relies on dsn() returning None in test builds, exercised separately.)
        assert!(dsn().is_none(), "pre-condition: no DSN in test build");
        // We can't call init_desktop_with_minidump in a test because it installs a
        // panic hook and may spawn a child process. But by design it calls dsn()?
        // at the very top, so when dsn() is None it returns None with zero side
        // effects. We verify the pre-condition (dsn None) + the function signature
        // returns Option; that's sufficient without executing the monitor path.
        // The fact that no DSN is baked in is verified by the dsn_is_none_in_dev_builds
        // test; this test makes the relationship explicit.
    }
