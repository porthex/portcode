...
| 2     | Parallel execution + concurrency cap     | 🟦     | PR3: batch `task` calls run via `buffer_unordered(MAX_PARALLEL_AGENTS=4)`, results slotted back in tool_use order; per-run `ask_lock` serializes concurrent subagents' prompts; non-task tools unchanged; +3 tests; local green; review pending                             |
