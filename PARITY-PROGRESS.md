...
| 2     | Background tasks                         | 🟦     | PR5a (backend): `shell` `background:true` → `BackgroundRunner` on `ToolCtx` + `background.rs` registry + `BackgroundLauncher` (waits off-thread, emits `BackgroundTaskStarted`/`Finished` on the session channel, session Stop kills via abort) + shared `build_shell_command`/`format_shell_output`; +6 tests; local green. PR5b (frontend persistent listener + UI) next    |
