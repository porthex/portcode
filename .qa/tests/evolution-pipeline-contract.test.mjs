...
test("pipeline separates pre-implementation evolution from post-implementation verification", () => {
  assert.deepEqual(PREPARATION_STAGES.map(({ id }) => id), [
    "real-world-use-case-scout",
    "pre-implementation-risk-architect",
    "feature-brief-synthesizer",
  ]);
  assert.deepEqual(VERIFICATION_STAGES.map(({ id }) => id), [
...
test("use-case semantics reject automatic additions without strong evidence", () => {
  const report = {
    ...
    proposals: [{
      ...
      evidence: [{ type: "comparable-product", locator: "https://example.test/a", claim: "A supports it" }],
    }],
  };
...
test("feature brief cannot mutate or omit explicit original requirements", () => {
  const scout = { outcome: "ready", originalRequirements: [{ id: "REQ-001", text: "Attach files" }], proposals: [] };
  const risks = { outcome: "ready", risks: [] };
  const brief = { outcome: "ready", originalRequestVerbatim: "Mutated task", originalRequirements: [{ id: "REQ-001", text: "Upload files" }], proposalDecisions: [], finalRequirements: [] };
...
test("risk verification accounts for every frozen risk and blocked high risk forces review", () => {
  const register = { outcome: "ready", registerId: "risk-1", risks: [
    { id: "RISK-001", severity: "high" },
    { id: "RISK-002", severity: "low" },
  ] };
  const report = { outcome: "completed", riskRegisterId: "risk-1", verdicts: [
    { riskId: "RISK-001", status: "blocked" },
  ] };
  assert.ok(validateRiskVerificationSemantics(report, register).some((error) => /RISK-002/ig.test(error)));
...
test("blocked exploration coverage forces review even when frozen risks and findings pass", () => {
  const risks = { outcome: "completed", verdicts: [{ riskId: "RISK-001", status: "verified-safe" }] };
  const exploration = { outcome: "completed", coverage: { planned: 4, selected: 4, executed: 3, passed: 3, observationsRecorded: 0, blocked: 1, notApplicable: 0 } };
...
test("risk verification requires evidence before dismissing a frozen risk as not applicable", () => {
  const register = { outcome: "ready", registerId: "risk-na", risks: [{ id: "RISK-001", severity: "high" }] };
  const report = {
    outcome: "completed",
    riskRegisterId: "risk-na",
    verdicts: [{ riskId: "RISK-001", status: "not-applicable", rationale: "Claimed unreachable", evidence: {} }],
    summary: { total: 1, verifiedSafe: 0, findings: 0, notApplicable: 1, blocked: 0, coverageGate: "pass" },
  };
  assert.ok(validateRiskVerificationSemantics(report, register).some((error) => /objective evidence/i.test(error)));
});
...
test("runner derives the finding gate from configured severities, never the agent policy", () => {
  const report = {
    ...
    verdicts: [
      { candidateId: "RV-001", disposition: "confirmed", finalSeverity: "critical" },
    ],
  };
...
test("runner rejects missing evidence artifacts and accepts contained files", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "evidence", "edge-case-explorer", "real.png"), "fresh-stage-evidence");
    assert.deepEqual(await validateEvidenceArtifacts({ evidence: { screenshots: ["evidence/edge-case-explorer/real.png"], trace: null } }, root, "edge-case-explorer"), []);
    assert.ok((await validateEvidenceArtifacts({ evidence: { screenshots: ["screenshots/real.png"], trace: null } }, root, "edge-case-explorer"))
      .some((error) => /not owned by stage/i.test(error)));
    assert.ok((await validateEvidenceArtifacts({ evidence: { screenshots: ["evidence/edge-case-explorer/missing.png"], trace: null } }, root, "edge-case-explorer"))
      .some((error) => /does not exist/i.test(error)));
    assert.ok((await validateEvidenceArtifacts({ evidence: { screenshots: ["../escape.png"], trace: null } }, root, "edge-case-explorer"))
...
test("preparation provenance pins task and upstream reports while verification pins the frozen brief and risks", () => {
  ...
  assert.deepEqual(validateStageProvenance("feature-brief-synthesizer", {
    outcome: "ready",
    provenance: {
  ...
  assert.ok(validateStageProvenance("post-implementation-risk-verifier", {
    outcome: "completed",
    provenance: { featureBriefSha256: "wrong", riskRegisterSha256: expected.riskRegisterSha256 },
  }, expected).some((error) => error.includes("featureBriefSha256")));
});
