...
This repo ships a committed **code knowledge graph** at `graphify-out/graph.json` — a map of the
codebase's files, symbols, and relationships that you (or an AI assistant) can query instead of
grepping. It is generated by [graphify](https://github.com/safishamsi/graphify).
...
`Cargo.toml`, or the CI workflow references it), and it is **not** a quality gate. The committed
graph is purely a convenience. The `.claude/settings.json` hooks only _suggest_ querying the graph
during AI-assisted sessions; they are advisory and never block a command. To turn the suggestions
off, remove the `hooks` block in `.claude/settings.json` (or delete the file).
